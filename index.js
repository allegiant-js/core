...
class ClientRequest extends httpDuplex {
    constructor(req, res, app) {
        ...
        if (this.method == 'POST') {
            ...
            if (!this.readable) {
                this.body = typeof decoders[contentType] === 'function' ? decoders[contentType](this.req.body) : this.req.body;
                ...
                this.readPostData(function(content) {
                    this.body = typeof decoders[contentType] === 'function' ? decoders[contentType](content) : content;
...
class App {
    ...
    moduleLoaded(name) {
        return typeof this.modules[name] !== 'undefined' ? this.modules[name] : false;
    }

    loadModules(module, options) {
        var config;

        if (Array.isArray(module)) {
            for (var i=0, max=module.length; i < max; i++) {
                this.loadModules.apply(this, module[i]);
            }
        ...
        config = this.modules[module] = require(module).Configure(this, options);

        if (expect(config.required, false))
            this.loadModules(Object.entries(config.required));
    }
    ...
    binder() {
        ...
        // collapses 'any' handlers into every event, so there isn't an unneeded array map operation every request
